...
async def test_create_dispute(db_session):
    ...
    # Setup test data
    buyer = User(phone_number="0241234567", hashed_password="hash", full_name="Buyer", role=UserRole.BUYER, status=UserStatus.ACTIVE)
    seller = User(phone_number="0247654321", hashed_password="hash", full_name="Seller", role=UserRole.SELLER, status=UserStatus.ACTIVE)
...
async def test_dispute_resolution_refunds_payer_after_required_approvals(db_session):
    buyer = User(phone_number="0241234567", hashed_password="hash", full_name="Buyer", role=UserRole.BUYER, status=UserStatus.ACTIVE)
    seller = User(phone_number="0247654321", hashed_password="hash", full_name="Recipient", role=UserRole.SELLER, status=UserStatus.ACTIVE)
    ...
    await db_session.refresh(admin_action)
...
async def test_dispute_resolution_accepts_legacy_buyer_wins_alias(db_session):
    buyer = User(phone_number="0241234568", hashed_password="hash", full_name="Buyer", role=UserRole.BUYER, status=UserStatus.ACTIVE)
    seller = User(phone_number="0247654328", hashed_password="hash", full_name="Recipient", role=UserRole.SELLER, status=UserStatus.ACTIVE)
